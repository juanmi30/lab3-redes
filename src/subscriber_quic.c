// subscriber_quic.c - Envía SUB ... y recibe eventos en el mismo stream
// Ejecutar: subscriber_quic.exe -target:127.0.0.1 -port:4433
...
/* helper: enviar copiando al heap */
...
QUIC_STATUS QUIC_API ConnCb(HQUIC Connection, void* Context, QUIC_CONNECTION_EVENT* Event) {
    ...
    if (Event->Type==QUIC_CONNECTION_EVENT_CONNECTED) {
        ...
        app->ready = 1;
        printf("Conectado. Escriba partidos: (ej: ARGvsBRA COLvsURU)\n");
    }
    return QUIC_STATUS_SUCCESS;
}
...
int main(int argc, char** argv) {
    ...
    QUIC_REGISTRATION_CONFIG reg={"sub-pubsub",QUIC_EXECUTION_PROFILE_LOW_LATENCY};
    ...
    // Pedir temas y enviar SUB cuando el stream esté listo
    char topics[MAX_LINE];
    fgets(topics,sizeof(topics),stdin);
    size_t n=strcspn(topics,"\r\n"); topics[n]='\0';

    while(!app.ready) Sleep(10);
    char subline[MAX_LINE]; snprintf(subline,sizeof(subline),"SUB %s",topics);
    send_copy(app.Api, app.Stream, subline, (uint32_t)strlen(subline));

    printf("Suscrito. Esperando eventos...\n");
    getchar(); // mantener proceso vivo hasta Enter
